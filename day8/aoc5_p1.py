...
def processFile(filePath):
    ...
    with open(filePath, 'r') as file:
        ...
        for char, points in coordinates.items():
            for i in range(len(points)-1):
                for j in range(i+1, len(points)):
                    antinodeI = computeAntinode(points[i], points[j])
                    if antinodeI not in occupied and isValidPoint(antinodeI, ROWS, COLS):
                        res += 1
                        occupied.add(antinodeI)
                    antinodeJ = computeAntinode(points[j], points[i])
                    if antinodeJ not in occupied and isValidPoint(antinodeJ, ROWS, COLS):
                        res += 1
                        occupied.add(antinodeJ)
